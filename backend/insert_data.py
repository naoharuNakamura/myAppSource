...
# データベース接続
# -------------------------------------------------------------
# ※空の .accdb ファイルを事前に作成し、そのパスを指定してください
DB_PATH = "database.db" # ファイル名だけでOK
conn = sqlite3.connect(DB_PATH)
cursor = conn.cursor()

# SQLiteで外部キー制約(CASCADE)を有効化
cursor.execute("PRAGMA foreign_keys = ON;")

# -------------------------------------------------------------
# 1. テーブルの自動作成（初期化）
# -------------------------------------------------------------
print("🧹 既存のテーブルをリセットし、新しく作成中...")

# 削除順序: 依存関係が深い（子）テーブルから先に消す
tables_to_drop = ["t_user_review_history", "t_user_favorite", "m_user", "m_restaurant"]
...
# SQLiteではテーブルの削除順をそこまで厳密に気にしなくて良い場合が多いですが、
# 念のため依存関係を考慮して削除します。
cursor.execute("DROP TABLE IF EXISTS t_user_review_history")
cursor.execute("DROP TABLE IF EXISTS t_user_favorite")
cursor.execute("DROP TABLE IF EXISTS m_user")
cursor.execute("DROP TABLE IF EXISTS m_restaurant")
...
cursor.execute("""
    CREATE TABLE m_restaurant (
        ...
        restaurant_genre TEXT,
        restaurant_price_range TEXT,
        restaurant_area TEXT,
        restaurant_address TEXT,
        restaurant_open_hour TEXT,
        restaurant_close_hour TEXT,
        restaurant_phone TEXT,
        restaurant_url TEXT,
        restaurant_closed_days TEXT
    );
""")
...
cursor.execute("""
    CREATE TABLE t_user_favorite (
        ...
        FOREIGN KEY (restaurant_id) REFERENCES m_restaurant(restaurant_id) ON DELETE CASCADE
    );
""")

# t_user_review_history テーブル
cursor.execute("""
    CREATE TABLE t_user_review_history (
...
# -------------------------------------------------------------
# 0. メモサンプルの準備と、評価・データを一時保持するコンテナ
# -------------------------------------------------------------
memo_pool = [
    "美味しかった！リピート確定。",
    "コスパが非常に良いお店。",
...
# レストランごとの評価（1〜5の整数）を蓄積する辞書
rest_rating_dict = {i: [] for i in range(1, 20000)}

m_user_list = []
t_user_restaurant_list = []

# -------------------------------------------------------------
# 1. 先にユーザーデータと中間テーブル（評価・メモ）のシミュレーション生成
# -------------------------------------------------------------
print("🔑 ユーザーデータと中間テーブルの評価・メモをシミュレート中...")
...
for i in range(1, 1000):
    ...
    # ユーザーマスタ用リストに蓄積
    m_user_list.append((u_id, hashed_pwd, name, email, phone))
    
    # 各ユーザーがアクション（お気に入り・評価・メモ）を起こすレストランの数
    # ※Accessの負荷を考慮し最大50件程度に調整していますが、必要に応じて増やせます
    target_count = random.randint(0, 50)
    fav_ids = random.sample(range(1, 20000), target_count)
    
    for rest_id in fav_ids:
        # お気に入りかどうか（50%の確率）
        is_favorite = random.choice([0, 1])
        
        # お気に入りでなくても、30%の確率でランダムにメモを持つ
        user_memo = random.choice(memo_pool) if random.random() < 0.3 else None
        
        # 1〜5の整数でユーザー評価を生成
        user_rating = random.randint(1, 5)
        
        # レストランIDごとに評価値を集計用辞書に貯める
        rest_rating_dict[rest_id].append(user_rating)
        
        # 中間テーブル用リストに蓄積
        t_user_restaurant_list.append((u_id, rest_id, is_favorite, user_rating, user_memo))


# -------------------------------------------------------------
# 2. m_restaurant（レストランデータ 20,000件）の生成
# -------------------------------------------------------------
restaurants_list = []
...
for i in range(1, 20000):
    ...
    # 🌟 変更点: 中間テーブルの評価の平均値を計算して入れる
    if rest_rating_dict[i]:
        final_rating = round(sum(rest_rating_dict[i]) / len(rest_rating_dict[i]), 1)
    else:
        # 万が一、誰からも評価されなかったレストランは元のロジックで初期値を設定
    ...
    restaurants_list.append((
        name, image, final_rating, genre, price, area, 
        open_hour, close_hour, address, phone, url, closed_days
    ))


# -------------------------------------------------------------
# 3. データベースへの一括インサート（ここですべて一瞬で書き込みます）
# -------------------------------------------------------------
print("💾 データベース（Access）へ全データを一括書き込み中...")

# m_restaurant の一括インサート（★カラム名を正しく合わせました）
cursor.executemany("""
    INSERT INTO m_restaurant (
        restaurant_name, restaurant_img, restaurant_rating, restaurant_genre, restaurant_price_range, restaurant_area, 
        restaurant_open_hour, restaurant_close_hour, restaurant_address, restaurant_phone, restaurant_url, restaurant_closed_days
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
""", restaurants_list)
conn.commit()
...
# 1. お気に入り管理テーブルへのインサート
# リストから (user_id, restaurant_id, is_favorite) だけを抽出してインサート
favorite_data = [(item[0], item[1], item[2]) for item in t_user_restaurant_list]
cursor.executemany("""
    INSERT INTO t_user_favorite (user_id, restaurant_id, is_favorite) 
    VALUES (?, ?, ?);
""", favorite_data)

# 2. レビュー履歴テーブルへのインサート
# リストから (user_id, restaurant_id, user_rating, user_memo) を抽出
# ※created_at は DEFAULT 現在時刻が入るため指定不要
review_data = [(item[0], item[1], item[3], item[4]) for item in t_user_restaurant_list]
cursor.executemany("""
    INSERT INTO t_user_review_history (user_id, restaurant_id, user_rating, user_memo) 
    VALUES (?, ?, ?, ?);
""", review_data)
...
print("✨ 成功: ユーザー評価の平均値を反映したレストランマスタ作成、および独立したメモデータのインサートが完了しました！")